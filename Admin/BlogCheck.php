        ...
        <div class="container-graph">
            <form>
              ...
              if (isset($_GET['id'])) {
                ...
                $sql="SELECT Content FROM blog WHERE Blog_ID='$contentID'";
                $result = $con->query($sql);

                if ($result->num_rows>0) {
                  $cont=$result->fetch_assoc()["Content"];
                  
                  echo '<textarea name="message" rows=30 cols=30 class="message" style="margin-right:15px;">'.$cont.'</textarea>';
                }else{
            ...
            <div class="ContentEligible" style="margin: 10px 0px 0px 10px; padding: 10px;">
                <form method="POST" action="Approval.php">
                <label>
                    <input type="radio" name="approval" value="approved">
                        Approved
                    </label>
                    <label style="margin-left: 15px; padding: 1px;">
                        <input type="radio" name="approval" value="denied">
                        Denied
                    </label>
                    <div class="contentdone" style="margin: 20px 0px 0px 8px;">
                        <input type="hidden" name="blog_id" value="<?php echo $contentID; ?>">
                        <input type="submit" name="Checked" value="Done">
                    </div>
                </form>
              </div>
        </div>
